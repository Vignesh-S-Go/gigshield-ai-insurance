...
import { Search, Plus, Download, ChevronDown, Loader2 } from 'lucide-react';
import Header from '../components/Header';
import Modal from '../components/Modal';
import useStore from '../store/useStore';
import { formatCurrency, getStatusColor, getPlanColor, formatDate, exportToCSV } from '../utils/helpers';
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {['Basic', 'Standard', 'Pro'].map(plan => {
          ...
          return (
            <div key={plan} className="glass-card rounded-2xl p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-dark-400">{plan} Plan</p>
                  <p className="text-2xl font-bold text-dark-800 dark:text-dark-200 mt-1">{count}</p>
                </div>
                <div className={`px-3 py-1.5 rounded-xl text-xs font-semibold ${getPlanColor(plan)}`}>
                  ₹{planDetails[plan].premium}/week
                </div>
              </div>
              <div className="mt-2 text-xs text-dark-400">Max payout: {formatCurrency(planDetails[plan].maxPayout)}</div>
            </div>
          );
        })}
      </div>
            ...
            <tbody className="divide-y divide-dark-50 dark:divide-dark-800">
              {filtered.slice(0, 20).map(policy => (
                <tr key={policy.id} className="table-row">
                  <td className="px-5 py-4 text-sm font-mono text-dark-500">{policy.id}</td>
                  <td className="px-5 py-4 text-sm font-medium text-dark-800 dark:text-dark-200">{policy.workerName}</td>
                  <td className="px-5 py-4">
                    <span className={`badge ${getPlanColor(policy.planType)}`}>{policy.planType}</span>
                  </td>
                  <td className="px-5 py-4 text-sm font-semibold text-dark-800 dark:text-dark-200">{formatCurrency(policy.premium)}/wk</td>
                  <td className="px-5 py-4 text-sm text-dark-600 dark:text-dark-400">{formatCurrency(policy.maxPayout)}</td>
                  <td className="px-5 py-4">
                    <span className={`badge ${getStatusColor(policy.status)}`}>{policy.status}</span>
                  </td>
                  <td className="px-5 py-4 text-sm text-dark-500">{formatDate(policy.renewalDate)}</td>
                </tr>
              ))}
            </tbody>
          </table>
